...
extension CoinService {
    ...
    static func fetchCoins() {
        ...
        APIClient.shared.GET(endpoint: endpoint) { result in
            switch result {
            case .success(let json):
                DispatchQueue.global(qos: .background).async {
                    //let coins = CoinService.parseCoinsFromJson(json)
                    
                    DispatchQueue.main.async {
                        
                        //store.dispatch(action: SetAvailableCoins(coins: coins))
                    }
                }
            case .failure:
                break
...
extension CoinService {
    ...
    static func parseCoinsFromJson(_ json: Any) -> [Coin] {
        guard let arrayValue = json as? [Any] else {
            return []
        }
